...
ALTER TABLE usr ALTER COLUMN idx SET DEFAULT NEXTVAL('usr_idx_seq');
...
CREATE TABLE site(
    idx     SERIAL          CONSTRAINT site_pk                  PRIMARY KEY
...
ALTER TABLE site ALTER COLUMN idx SET DEFAULT NEXTVAL('site_idx_seq');
...
COMMENT ON COLUMN site.reg_id   IS '웹사이트 생성자';
COMMENT ON COLUMN site.reg_dt   IS '웹사이트 생성일';
COMMENT ON COLUMN site.chg_id   IS '웹사이트 변경자';
...
CREATE TABLE header(
    idx       SERIAL          CONSTRAINT header_pk        PRIMARY KEY
  , site_idx  INT             CONSTRAINT header_site_fk   REFERENCES site(idx) ON DELETE SET NULL
  , key       VARCHAR(255)
  , val       VARCHAR(255)
);
ALTER TABLE header ALTER COLUMN idx SET DEFAULT NEXTVAL('header_idx_seq');
...
CREATE TABLE formbody(
    idx       SERIAL          CONSTRAINT formbody_pk      PRIMARY KEY
  , site_idx  INT             CONSTRAINT formbody_site_fk REFERENCES site(idx) ON DELETE SET NULL
  , key       VARCHAR(255)
  , val       VARCHAR(255)
);
ALTER TABLE formbody ALTER COLUMN idx SET DEFAULT NEXTVAL('formbody_idx_seq');
