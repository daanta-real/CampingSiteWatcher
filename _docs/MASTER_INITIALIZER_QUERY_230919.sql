...
-- 회원등급 테이블의 생성
CREATE SEQUENCE grd_idx_seq START WITH 1;
CREATE TABLE grd(
      idx   INTEGER    CONSTRAINT grd_pk           DEFAULT NEXTVAL('grd_idx_seq') PRIMARY KEY
    , id    VARCHAR(6) CONSTRAINT grd_id_not_null  CHECK (id IS NOT NULL)
    , nm    VARCHAR(4) CONSTRAINT grd_nm_not_null  CHECK (nm IS NOT NULL)
);
COMMENT ON TABLE  grd     IS '회원-등급';
COMMENT ON COLUMN grd.idx IS '권한 idx';
COMMENT ON COLUMN grd.id  IS '권한 id';
COMMENT ON COLUMN grd.nm  IS '권한 명';

-- 회원 테이블의 생성
CREATE SEQUENCE usr_idx_seq START WITH 1;
CREATE TABLE usr(
...
-- 웹사이트 테이블 생성
CREATE SEQUENCE site_idx_seq START WITH 1;
CREATE TABLE site(
...
-- "header" 테이블 생성
CREATE SEQUENCE header_idx_seq START WITH 1;
CREATE TABLE header(
...
-- "formbody" 테이블 생성
CREATE SEQUENCE formbody_idx_seq START WITH 1;
...
/* GRD
사실 회원등급은 사이트 규모가 커지면 필요한 것들이라 지금 쓸모 없다.
하지만, 후일을 위해 미리 추가해 놓는다.
*/
SELECT SETVAL('grd_idx_seq', 6);
INSERT INTO grd
    (idx, id, nm)
VALUES
    (1, 'admin' , '운영자'),
...
VALUES
    (0, 1, 'asdf3', '1@naver.com'  , '1234', 'p1'),
    (1, 2, 'qf3r3', '2@daum.net'   , '2345', 'p2'),
    (2, 3, 'wg44g', '3@hotmail.com', '3456', 'p2'),
    (3, 4, 'dfp93', '4@naver.com'  , '4567', 'p4'),
    (4, 5, 'dd24f', '5@nate.com'   , '5678', 'p5');
...
/* HEA */
SELECT SETVAL('header_idx_seq', 7);
INSERT INTO header
...
/* BDY */
SELECT SETVAL('formbody_idx_seq', 5);
